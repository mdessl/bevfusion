        ...
        **kwargs,
        ...
        if self.save_embeddings:
            ...
            if "lidar" in self.encoders:
                lidar_feat = self.extract_features(points, "lidar")
                lidar_path = os.path.join(self.save_path, f"{token}_lidar.pth")
                print(lidar_path)
                torch.save(lidar_feat, lidar_path)

            if "radar" in self.encoders:
                radar_feat = self.extract_features(radar, "radar")
                radar_path = os.path.join(self.save_path, f"{token}_radar.pth")
                torch.save(radar_feat, radar_path)
